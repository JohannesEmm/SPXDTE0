...
#TODO: CLosed indicator not anymore reported since some time in 2020/2021
#create new "closed" column
trades <- suppressWarnings(trades %>% group_by(description) %>% mutate(time_starting_close=max(min(timestamp[quantity<0]), min(timestamp[quantity>0]))) %>% mutate(time_numerical=as.numeric(timestamp), openCloseIndicator=ifelse(as.numeric(timestamp)>=time_starting_close, "C", "O")) %>% select(-time_starting_close))
...
p_candlestick_trades <- ggplot(spx_1min_fortrades %>% mutate(Close=ifelse(new_day==1,NA,Close)), aes(x = timestamp_chr, y = Close, group = 1)) +  
  ...
  #Credit Spreads
  geom_segment(data=trades_strategies %>% filter(str_detect(type, "Credit Spread")), aes(x = timestamp_chr, y = strike, xend = timestamp_chr_closed, yend = strike, colour = type, alpha=as.character(sign(PnL_strategies_recomputed))), size=2) +
  geom_segment(data=trades_strategies %>% filter(str_detect(type, "Credit Spread")), aes(x = timestamp_chr, y = strike_long, xend = timestamp_chr_closed, yend = strike_long, colour = type, alpha=as.character(sign(PnL_strategies_recomputed))), size=0.5) + 
  xlab("") + ylab("SPX & PnL/contract") + 
  scale_colour_manual(values = c("Put Credit Spread"="blue", "Call Credit Spread"="orange", "Iron Butterfly"="green"), labels=c("Put Credit Spread"="Put Credit Spread", "Call Credit Spread"="Call Credit Spread", "Iron Butterfly"="Iron Butterfly"), name="Strategy") + scale_alpha_manual(values = c("-1"=0.5, "+1"=1, "0"=1)) + guides(alpha = FALSE)
#Add labels if gains or losses there
if(nrow(trades_strategies %>% filter(str_detect(type, "Credit Spread")) %>% filter(PnL>0))>0) p_candlestick_trades <- p_candlestick_trades + geom_label(data=trades_strategies %>% filter(str_detect(type, "Credit Spread")) %>% filter(PnL>0), aes(x = timestamp_chr_middle, y = strike + ifelse(putCall=="P", -shiftlabel,+shiftlabel), label=str_glue("{format(PnL_strategies_recomputed/contracts/100, digits = 2, nsmall = 2)}")), size=3, color="darkgreen", alpha=0.5)
if(nrow(trades_strategies %>% filter(str_detect(type, "Credit Spread")) %>% filter(PnL<0))>0) p_candlestick_trades <- p_candlestick_trades + geom_label(data=trades_strategies %>% filter(str_detect(type, "Credit Spread")) %>% filter(PnL<0), aes(x = timestamp_chr_middle, y = strike + ifelse(putCall=="P", -shiftlabel,+shiftlabel), label=str_glue("{format(PnL_strategies_recomputed/contracts/100, digits = 2, nsmall = 2)}")), size=3, color="darkred", alpha=0.5)
  #Add Iron Butterflys
...
print(ggarrange(p_candlestick_trades + theme(axis.title.y = element_text(margin = margin(t = 0, r = 15, b = 0, l = 0))), vix_plot + theme(axis.title.y = element_text(margin = margin(t = 0, r = 23, b = 0, l = 0))), p_PnL_bar + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + theme(axis.title.y = element_text(margin = margin(t = 0, r = 1, b = 0, l = 0))), p_total_relative + theme(axis.text.x=element_blank(), axis.ticks.x=element_blank()) + theme(axis.title.y = element_text(margin = margin(t = 0, r = 0, b = 0, l = 0))), nrow = 4, heights = c(.55,.15,.15,.15), common.legend = T, legend = "bottom", hjust=c(0, 0, 0, 0)))
...
shell(str_glue('pdftopng.exe -r 300 Trading_log_full.pdf - > Trading_log_full.png'))
################ END FIGURE ##################
