...
p_candlestick_trades <- ggplot(data = spx_1min_fortrades, aes(x = timestamp, y = Close)) +
  ...
  geom_segment(data=trades_strategies, aes(x = date_opened, y = strike, xend = date_closed, yend = strike, colour = putCall), size=2) + theme_tq() + scale_x_datetime(date_labels = '%e %b',date_breaks = '1 day', limits = c(x_min, x_max)) + geom_label(data=trades_strategies %>% filter(PnL>0), aes(x = date_opened, y = strike + ifelse(putCall=="P", -20,+20), label=str_glue("{contracts}x{credit}:+{round(PnL)}$")), size=3, color="darkgreen") + geom_label(data=trades_strategies %>% filter(PnL<0), aes(x = date_opened, y = strike + ifelse(putCall=="P", -20,+20), label=str_glue("{contracts}x{credit}:{round(PnL)}$")), size=3, color="darkred")  + xlab("") + ylab("SPX")
print(p_candlestick_trades)

#Bar chart per day
p_PnL_bar <- ggplot(data = trades_strategies %>% group_by(day_opened) %>% summarize(PnL=sum(PnL))) +
  geom_line(data = spx_1min_fortrades, aes(x = timestamp, y = Close), color="white", alpha=0) + theme_tq() +  scale_x_datetime(date_labels = '%e %b',date_breaks = '1 day', limits = c(x_min, x_max)) +
  geom_bar(aes(x = day_opened, y = PnL, fill=as.character(sign(PnL))), width = 5e4, stat = "identity") + xlab("") + ylab("PnL")  + scale_fill_manual(breaks = c(-1,+1), values = c("darkred", "darkgreen")) + theme(legend.position = "none") + geom_text(aes(x = day_opened, y = sign(PnL)*(abs(PnL)+200), label=sprintf('%+.0f$', PnL)))
print(p_PnL_bar)

#Line chart total
p_total_line <- ggplot(data = trades_strategies %>% group_by(day_opened) %>% summarize(PnL=sum(PnL))) +
  geom_line(data = spx_1min_fortrades, aes(x = timestamp, y = Close), color="white", alpha=0) + theme_tq() +  scale_x_datetime(date_labels = '%e %b',date_breaks = '1 day', limits = c(x_min, x_max)) +
  geom_line(aes(x = day_opened, y = cumsum(PnL)), color="blue", size=2) + xlab("") + ylab("Total [$]") + theme(legend.position = "none") + geom_text(aes(x = day_opened, y = cumsum(PnL)+200, label=sprintf('%+.0f$', cumsum(PnL))), color="blue")
print(p_total_line)

ggarrange(p_candlestick_trades, p_PnL_bar, p_total_line, nrow = 3, heights = c(.6,.2,.2), common.legend = T, legend = "bottom", hjust=c(0, 0, 0))
